...
class RealWorldDataset(Dataset):
    ...
    def __init__(
        ...
        self.history_frame_ids = []
        self.track_indices = []
        self.demo_frame_ids = []  # 存储每个demo的完整frame_ids
        self.demo_tcp_data = []   # 新增：存储每个demo的所有TCP数据
        self.projectors = {}
        ...
        for i in range(self.num_demos):
            ...
            for cam_id in cam_ids:
                ...
                tcp_dir = os.path.join(demo_path, "cam_{}".format(cam_id), 'tcp')
                tcp_data_list = []
                for frame_id in frame_ids:
                    tcp = np.load(os.path.join(tcp_dir, "{}.npy".format(frame_id)))[:7].astype(np.float32)
                    tcp_data_list.append(tcp)
                tcp_data_array = np.array(tcp_data_list)  # shape: (num_frames, 7)
                # get samples according to num_obs and num_action
                obs_frame_ids_list = []
                action_frame_ids_list = []
                history_frame_ids_list = []
                ...
                self.history_frame_ids += history_frame_ids_list
                self.track_indices += track_indices_list
                self.demo_frame_ids += [frame_ids] * len(obs_frame_ids_list)  # 存储每个sample对应的完整frame_ids
                self.demo_tcp_data += [tcp_data_array] * len(obs_frame_ids_list)  # 存储每个sample对应的TCP数据
    ...
    def _generate_gripper_tracks_from_tcp(self, tcp_data_array, track_idx, projector, cam_id):
        """
        Returns:
            track_array: shape (track_idx+1, 2, 3) - [timestep, [left/right], [normalized_x, normalized_y, normalized_z]]
        """
        track_points = []
        
        for i in range(track_idx + 1):
            tcp = tcp_data_array[i]  # shape: (7,)
            tcp_position = tcp[:3]  
            tcp_rotation = tcp[3:]  
            ...
            left_tcp_camera = projector.project_tcp_to_camera_coord(left_tcp_full, cam_id)
            right_tcp_camera = projector.project_tcp_to_camera_coord(right_tcp_full, cam_id)
            ...
            track_point = np.array([left_gripper_pos_normalized, right_gripper_pos_normalized])  # shape: (2, 3)
            track_points.append(track_point)
        
        # shape (track_idx+1, 2, 3)
    ...
    def __getitem__(self, index):
        ...
        history_frame_ids = self.history_frame_ids[index] 
        track_idx = self.track_indices[index]
        frame_ids = self.demo_frame_ids[index]  
        tcp_data_array = self.demo_tcp_data[index]  
        ...
        track_array = self._generate_gripper_tracks_from_tcp(
            tcp_data_array, track_idx, projector, cam_id
        )
        ...
        relative_actions = torch.from_numpy(relative_actions).float()
        relative_actions_normalized = torch.from_numpy(relative_actions_normalized).float()
        point_tracks = torch.from_numpy(track_array).float()
